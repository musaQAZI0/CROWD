...
async function demonstrateMarketingAPI(token, user) {
    ...
    try {
        ...
        const campaignsResponse = await fetch('http://localhost:3001/api/marketing/campaigns', {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            }
    ...
    try {
        ...
        const socialResponse = await fetch('http://localhost:3001/api/marketing/social-stats', {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            }
